...
class ReportEditorScreenState extends State<ReportEditorScreen>
  ...
  void initState() {
    ...
    _customRepo = CustomCategoryRepository(client);
  ...
  void reloadCustomCategories() {
    _customRepo.getForGroup(widget.group.id).then((cats) {
  ...
  Future<void> _reloadPeriods() async {
    ...
    final customCats = await _customRepo.getForGroup(widget.group.id);
    if (!mounted) return;
                        ...
                        child: Column(
                          children: [
                            ...
                            ..._customCategories.map((cat) {
                              final val = _report.getCustomValue(cat.id);
                                  ...
                                  children: [
                                    ...
                                    SliderTheme(
                                      ...
                                      child: Slider(
                                        min: 0,
                                        max: cat.weekMax.toDouble(),
                                        value: val.toDouble().clamp(0, cat.weekMax.toDouble()),
                                        onChanged: (v) => setState(() => _report.setCustomValue(cat.id, v.round())),
                                      ),
                                    ),
                                    Padding(
                                      padding: const EdgeInsets.only(bottom: 10),
                                      child: Row(
                                        ...
                                        children: [
                                          _CounterBtn(
                                            icon: Icons.remove,
                                            color: color,
                                            onTap: val > 0 ? () => setState(() => _report.setCustomValue(cat.id, val - 1)) : null,
                                          ),
                                          Container(
                                            margin: const EdgeInsets.symmetric(horizontal: 18),
                                            padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 6),
                                          ...
                                          _CounterBtn(
                                            icon: Icons.add,
                                            color: color,
                                            onTap: val < cat.weekMax ? () => setState(() => _report.setCustomValue(cat.id, val + 1)) : null,
                                          ),
                                        ],
                                      ),
                                    ),
                                    Builder(builder: (_) {
                                      final quickVals = [0.25, 0.5, 0.75, 1.0]
                                          .map((f) => (cat.weekMax * f).round())
                                          .toSet()
                                          .toList();
                                      return Row(
                                        children: quickVals.map((quickVal) {
                                          final isSelected = val == quickVal;
                                          return Expanded(
                                            child: Padding(
                                              padding: const EdgeInsets.only(right: 6),
                                              child: GestureDetector(
                                                onTap: () => setState(() => _report.setCustomValue(cat.id, quickVal)),
                                                child: AnimatedContainer(
